...
class LocalizationTests(unittest.TestCase):
    ...
    def test_normalize_chinese_punctuation(self):
        self.assertEqual(i18n.normalize_chinese("用于维护文档,方便协作."), "用于维护文档，方便协作。")
    ...
    def test_translate_cache_and_source_invalidation(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            rows = [{"repo": "a/b", "track": "开发工具", "what": "开源开发者工具与效率项目", "use": "Writes documentation."}]
            ...
            self.assertTrue(i18n.valid_chinese(out[0]["use"]))
            out, stats = i18n.localize_rows(rows, {}, root=temp, translator=translate)
    ...
    def test_invalid_translation_uses_truthful_chinese_fallback(self):
        with tempfile.TemporaryDirectory() as temp:
            rows = [{"repo": "a/b", "track": "开发工具", "what": "开源开发者工具与效率项目", "use": "A useful tool for software developers."}]
            out, stats = i18n.localize_rows(
                rows, {}, root=temp,
                translator=lambda items: {"0": "A useful tool for software developers."},
            )
            self.assertTrue(i18n.valid_chinese(out[0]["use"]))
            self.assertIn("官方仓库说明", out[0]["use"])
    ...
    def test_manual_override(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            out, stats = i18n.localize_rows(rows, {}, root=temp, translator=lambda _: self.fail("unexpected translator"))
    ...
    def test_unsupported_provider_fails_closed(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            with self.assertRaises(i18n.LocalizationError):
                i18n.localize_rows(rows, {"translation_provider": "retired-api"}, root=temp)

    def test_missing_source_is_chinese(self):
        with tempfile.TemporaryDirectory() as temp:
            rows = [{"repo": "a/b", "track": "开发工具", "what": "开发工具", "use": ""}]
            out, stats = i18n.localize_rows(rows, {}, root=temp, translator=lambda _: self.fail("unexpected translator"))
            self.assertTrue(i18n.valid_chinese(out[0]["use"]))
