...
from weekly_common import (
    ARCHIVE_DIR, CFG, CREATED_CUTOFF, DATA_DIR, MIN_GROWTH, ROOT,
    RUN_DATE, TODAY, WINDOW_START, classify,
)
from weekly_i18n import localize_rows, assert_chinese_uses
from weekly_render import generate_csv, generate_html
from weekly_sources import (
    closest_point, discover_candidates, github_repo, parse_oss_history,
    snapshot_current,
)
...
def main() -> int:
    ...
    ai_rows = sorted((r for r in results if r["track"] == "AI Agent"), key=lambda r: r["growth"], reverse=True)
    other_rows = sorted((r for r in results if r["track"] != "AI Agent"), key=lambda r: r["growth"], reverse=True)
    final_rows = ai_rows + other_rows
    for rank, row in enumerate(final_rows, 1):
        row["rank"] = rank

    # Localize only after ranking validation; never change numeric data or classification.
    final_rows, localization = localize_rows(final_rows, CFG, root=ROOT)
    assert_chinese_uses(final_rows)
    for row in final_rows:
        row.pop("use_source", None)
